...
public class TeacherViewEditor {
    ...
    public TeacherViewEditor(Teacher teacher, Grid grid) {
        ...
        logger.debug("TeacherViewEditor constructor invoked; " + teacher.getId());
        ...
        birthDay.setValue(teacher.getBirthDay() == null ? null : LocalDate.parse(new SimpleDateFormat("yyyy-MM-dd").format(teacher
            .getBirthDay())));
        birthDay.setResolution(DateResolution.DAY);
        binder.forField(birthDay)
            .withValidator(new DateRangeValidator("input Birth Day", LocalDate.of(1900, 1, 1), LocalDate.of(2002, 1, 1)))
    ...
    public void buttonBuild() {
        ...
        logger.debug("buttonBuild method (class TeacherViewEditor) invoked");
